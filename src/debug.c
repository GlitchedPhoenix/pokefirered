...
static void DebugAction_FlagsVars_Select(u8 taskId)
{
    ...
    if (gMain.newKeys & DPAD_ANY)
    {
        ...
        StringExpandPlaceholders(gStringVar1, sDebugText_FlagsVars_VariableHex);
        if (VarGetIfExists(gTasks[taskId].data[3]) == 65535) //Current value, if 65535 the value hasnt been set
...
{
    // Sound effect
    if (FlagGet(FLAG_WORLD_MAP_INDIGO_PLATEAU_EXTERIOR))
    {
        PlaySE(SE_PC_OFF);
...
#define SOUND_LIST_BGM \
    ...
    X(MUS_VS_DOG, "MUS-VS-DOG") \
    X(MUS_ABANDONED_SHIP, "MUS-ABANDONED-SHIP") \
    X(MUS_RS_VS_WILD, "MUS-RS-VS-WILD") \
